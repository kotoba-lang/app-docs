// docs.etzhayyim.com thin edge facade. OAuth/sync/cron run in AgentGateway MCP + pod-side LangServer.
...
const APP = "docs";
const NSID_PREFIX = "com.etzhayyim.apps.docs.";

export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    const url = new URL(req.url);
    if (url.pathname === "/health" || url.pathname === "/_app/meta") return json({ ok: true, actor: "did:web:docs.etzhayyim.com", nanoid: env.APP_NANOID ?? "r5ycqp6x", execution: "edge-proxy+agentgateway-mcp+langserver", businessLogic: "40-engine/kotoba/crates/kotoba-kotodama/py/src/kotodama/ingest/gworkspace_lite.py", bpmn: "etzhayyim-root/00-contracts/bpmn/com/etzhayyim/docs" });
